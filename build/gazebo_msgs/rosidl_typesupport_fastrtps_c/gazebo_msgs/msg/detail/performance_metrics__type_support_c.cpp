  ...
  void * untyped_ros_message)
{
  ...
  // Field name: sensors
  {
    ...
    if (!gazebo_msgs__msg__SensorPerformanceMetric__Sequence__init(&ros_message->sensors, size)) {
      return "failed to create array for field 'sensors'";
    }
    auto array_ptr = ros_message->sensors.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
...
static size_t _PerformanceMetrics__max_serialized_size(bool & full_bounded)
{
  return max_serialized_size_gazebo_msgs__msg__PerformanceMetrics(
    full_bounded, 0);
}
