...
class RslidarPacket(metaclass=Metaclass_RslidarPacket):
    ...
    SLOT_TYPES = (
        ...
        rosidl_parser.definition.Array(rosidl_parser.definition.BasicType('uint8'), 1248),  # noqa: E501
    )
    ...
    def __init__(self, **kwargs):
        ...
        if 'data' not in kwargs:
            self.data = numpy.zeros(1248, dtype=numpy.uint8)
        else:
            self.data = numpy.array(kwargs.get('data'), dtype=numpy.uint8)
            assert self.data.shape == (1248, )
    ...
    def data(self, value):
        if isinstance(value, numpy.ndarray):
            ...
            assert value.size == 1248, \
                "The 'data' numpy.ndarray() must have a size of 1248"
            self._data = value
            return
        if __debug__:
            ...
            assert \
                ...
                "The 'data' field must be a set or sequence with length 1248 and each value of type 'int' and each unsigned integer in [0, 255]"
